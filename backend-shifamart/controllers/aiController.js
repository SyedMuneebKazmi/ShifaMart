...
exports.chatWithAI = async (req, res) => {
  try {
    ...
    const sessionId = session_id || `user_${userId}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
...
exports.getFirstAid = async (req, res) => {
  try {
    ...
    const response = await pythonAPI.post('/api/first-aid', {
      symptoms: symptoms ? symptoms.map(s => s.toLowerCase().replace(/\s+/g, '_')) : undefined,
      emergency_type
    });
