...
class SeverityDetectionModel:
    ...
    def build_training_data(self, symptom_list: List[str]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # EMERGENCY cases: critical symptoms or dangerous combinations
        for _ in range(300):
            vector = np.zeros(n_symptoms)
            emergency_symptoms = [s for s in self.EMERGENCY_SYMPTOMS.keys() 
                                if s in self.symptom_to_idx]
            if emergency_symptoms:
                n_to_add = np.random.randint(1, 4)
                chosen = np.random.choice(emergency_symptoms, 
                                         min(n_to_add, len(emergency_symptoms)), 
                                         replace=False)
                for s in chosen:
                    vector[self.symptom_to_idx[s]] = 1
                X.append(vector)
    ...
    def predict_severity(self, symptoms: List[str], duration: str = None) -> Dict:
        ...
        for symptom in symptoms:
            if symptom in self.EMERGENCY_SYMPTOMS:
                emergency_score = self.EMERGENCY_SYMPTOMS[symptom]
