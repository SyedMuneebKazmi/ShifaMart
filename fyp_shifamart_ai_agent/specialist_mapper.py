...
class SpecialistMapper:
    ...
    DISEASE_SPECIALIST_MAP = {
        ...
        # Dental / oral / ENT-related (combined_dataset disease names)
        'cellulitis or abscess of mouth': 'ent',
        'abscess of mouth': 'ent',
        'teething syndrome': 'ent',
        'gingivitis': 'ent',
        'periodontitis': 'ent',
    ...
    SYMPTOM_SPECIALIST_MAP = {
        ...
        'toothache': 'ent',
        'ear_pain': 'ent',
        'pus_draining_from_ear': 'ent',
        'diminished_hearing': 'ent',
        'sore_throat': 'ent',
    }
    ...
    def get_specialist_for_disease(self, disease: str) -> Dict:
        ...
        # Prefer longer map keys first so e.g. "cellulitis or abscess of mouth" (ENT)
        # wins over the substring "cellulitis" alone (which maps to dermatologist).
        sorted_map_items = sorted(
            self.DISEASE_SPECIALIST_MAP.items(),
            key=lambda kv: len(kv[0]),
            reverse=True,
        )
        for disease_key, specialist_key in sorted_map_items:
            if disease_key in disease_lower:
                return self._get_specialist_info(specialist_key)
            # Shorthand only when the map key is not much longer than the disease name
            # (avoids "cellulitis" matching "cellulitis or abscess of mouth").
            if disease_lower in disease_key and len(disease_key) <= len(disease_lower) + 12:
                return self._get_specialist_info(specialist_key)
