...
class ConversationState(Enum):
    ...
    ASKING_MORE_SYMPTOMS = "asking_more_symptoms"
...
class EnhancedSymptomChecker:
    ...
    def process_message(self, session_id: str, user_message: str) -> Dict:
        ...
        if not user_message:
            ...
            # Allow "analyze" to skip straight to analysis (power user shortcut)
            if user_message.lower() in ['analyze', 'analyze now']:
                return self._analyze_and_show_results(session)
            return self._handle_duration(session, user_message)
        
        elif session.state == ConversationState.ASKING_SEVERITY:
            if user_message.lower() in ['analyze', 'analyze now']:
                return self._analyze_and_show_results(session)
            return self._handle_severity(session, user_message)
        
        elif session.state == ConversationState.ASKING_MORE_SYMPTOMS:
            return self._handle_more_symptoms(session, user_message)
    ...
    def _handle_symptom_collection(self, session: PatientSession, user_message: str) -> Dict:
        ...
        # Proactively ask clarifying questions once we have any symptom.
        # If NLP already extracted duration from the user's message, skip
        # duration and jump to severity follow-up. If both duration and
        # severity are already known (user came back from confirmation),
        ...
        if session.collected_symptoms:
            ...
            elif not session.symptom_severity:
                return self._ask_severity(session)
            else:
    ...
    def _ask_duration(self, session: PatientSession) -> Dict:
        ...
        if session.symptom_duration:
            # Duration already extracted from NLP - skip to severity follow-up
            return self._ask_severity(session)
    ...
    def _handle_duration(self, session: PatientSession, user_message: str) -> Dict:
        ...
        if 'skip' in msg_lower or "don't know" in msg_lower or "dont know" in msg_lower:
            session.symptom_duration = session.symptom_duration or 'Not specified'
            return self._ask_severity(session)
        ...
        # After duration, ask about severity before confirming
        return self._ask_severity(session)
    
    def _ask_severity(self, session: PatientSession) -> Dict:
        """Ask the patient to rate how severe their symptoms feel."""
        session.state = ConversationState.ASKING_SEVERITY
        symptom_list = [s.replace('_', ' ').title() for s in session.collected_symptoms]
        duration_text = (
            f"⏱️ **Duration:** {session.symptom_duration}\n\n"
            if session.symptom_duration else ""
        )
        return {
            'response': (
                f"📋 **Symptoms:** {', '.join(symptom_list)}\n"
                f"{duration_text}"
                "**How severe would you rate your symptoms overall?**\n"
                "_(Mild = manageable, Moderate = affects daily activities, "
                "Severe = hard to function, Very severe = urgent)_"
            ),
            'state': session.state.value,
            'suggestions': ['Mild', 'Moderate', 'Severe', 'Very severe'],
            'collected_symptoms': session.collected_symptoms
        }
    
    def _handle_severity(self, session: PatientSession, user_message: str) -> Dict:
        """Capture severity and move on to ask about additional symptoms."""
        ...
        for key, value in severity_map.items():
            if key in msg_lower:
                matched_severity = value
                break
        
        if not matched_severity and 'skip' not in msg_lower:
            nlp_result = self.nlp_processor.analyze_text(user_message)
            extracted_symptoms = nlp_result['symptoms']
        ...
        session.symptom_severity = matched_severity or user_message.strip()
        return self._ask_more_symptoms(session)
    
    def _ask_more_symptoms(self, session: PatientSession) -> Dict:
        """Ask if the patient has any other symptoms before analyzing."""
        session.state = ConversationState.ASKING_MORE_SYMPTOMS
        symptom_list = [s.replace('_', ' ').title() for s in session.collected_symptoms]
        return {
            'response': (
                f"📋 **So far:** {', '.join(symptom_list)}\n"
                f"⏱️ **Duration:** {session.symptom_duration or 'Not specified'}\n"
                f"📊 **Severity:** {session.symptom_severity or 'Not specified'}\n\n"
                "**Do you have any other symptoms?** If yes, please describe them. "
                "Otherwise reply **'no'** or **'analyze'** to continue."
            ),
            'state': session.state.value,
            'suggestions': ['No, analyze now', 'I have more symptoms', 'Fatigue', 'Nausea', 'Dizziness'],
            'collected_symptoms': session.collected_symptoms
        }
    
    def _handle_more_symptoms(self, session: PatientSession, user_message: str) -> Dict:
        """Handle the follow-up asking whether there are any additional symptoms."""
        msg_lower = user_message.lower().strip()
        
        negative_keywords = ['no', 'none', 'nope', 'nothing', 'that is all', "that's all",
                             'thats all', 'no more', 'analyze', 'done', 'finish', 'ok']
        if any(kw == msg_lower or kw in msg_lower.split() for kw in negative_keywords):
        ...
        if extracted_symptoms:
            ...
            if new_symptoms:
                ...
                all_symptoms_set = set(session.collected_symptoms)
                dangerous_combo_found = any(combo.issubset(all_symptoms_set) for combo in self.DANGEROUS_COMBINATIONS)
                critical_found = [s for s in session.collected_symptoms if s in self.CRITICAL_EMERGENCY_SYMPTOMS]
                if len(critical_found) >= 2 or dangerous_combo_found:
                    session.state = ConversationState.EMERGENCY_DETECTED
                    return self._handle_emergency(session, critical_found or list(session.collected_symptoms))
                ...
                return {
                    'response': (
                        f"✓ Added: **{', '.join(formatted)}**\n\n"
                        f"📋 **All symptoms:** {', '.join(all_symptoms)}\n\n"
                        "Any other symptoms? Reply **'no'** or **'analyze'** when ready."
                    ),
                    'state': session.state.value,
                    'suggestions': ['No, analyze now', 'I have more', 'Fatigue', 'Nausea'],
                    'collected_symptoms': session.collected_symptoms
                }
    ...
    def _show_confirmation(self, session: PatientSession) -> Dict:
        ...
        duration_text = f"⏱️ **Duration:** {session.symptom_duration}\n" if session.symptom_duration else ""
        severity_text = f"📊 **Severity:** {session.symptom_severity}\n" if session.symptom_severity else ""
